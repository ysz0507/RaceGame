    ...
    def __init__(self):
        ...
        while running: 
            ...
            if selected != displayed:
                selected %= len(maps)
                displayed = selected
                largeMap = pygame.image.load(os.path.join(os.getcwd(), "sources2D", "maps", maps[selected], 'optik.png')).convert()
            ...
            if play.checkLocation(*mouse): 
                if pressed:
                    ...
                    if firstTime:
                        ...
                        spiel = Rennspiel(Rennspiel.Dimension.TwoDmode, True, maps[selected])
                        #spiel = Rennspiel(Rennspiel.Dimension.ThreeDmode, False, "sachsenring")
                        spiel.mainLoop()
